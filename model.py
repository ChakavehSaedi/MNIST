...
class net_handler():
    ...
    def train_net(self, epochs, train_loader, verbos, log_file):
        ...
        for epoch in range(epochs):
            ...
            print("   -  Epoch %d: Accuracy %5.3f, Loss %5.3f -- time: %s"%(epoch+1, epoch_acc[epoch], epoch_loss[epoch], str(epoch_end - epoch_st)))
            with open(log_file, 'a') as log:
                log.write("   - Epoch %d: Accuracy %5.3f, Loss %5.3f -- time %s\n"%(epoch+1, epoch_acc[epoch], epoch_loss[epoch], str(epoch_end - epoch_st)))

        self.save_model()

        return epoch_acc, epoch_loss

    def test_net(self, test_loader, classes, plot, log_file):
        loss = 0
        acc = 0
        ...
        # Plot the last for elements
        plot(inputs, labels, max_indx)
